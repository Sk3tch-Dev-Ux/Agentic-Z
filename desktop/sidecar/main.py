...
class HealthResponse(BaseModel):
    status: str; sidecar_started_at: float; repo_root: str
...
class PreflightResponse(BaseModel):
    ...
    workshop_junction_ok: bool; overall_ok: bool; errors: list[str]; warnings: list[str]

class ModSummary(BaseModel):
    name: str; path: str; has_config_cpp: bool; has_pboprefix: bool
    has_p_junction: bool; last_modified: float
...
class StartRunResponse(BaseModel):
    run_id: str; skill: str; args: list[str]; started_at: float

class ActiveRun(BaseModel):
    run_id: str; mod_name: Optional[str]; skill: str; started_at: float; pid: int
...
def repo_info() -> RepoInfoResponse:
    return RepoInfoResponse(
        repo_root=str(REPO_ROOT), claude_dir=str(REPO_ROOT / ".claude"),
        workspace_dir=str(WORKSPACE_DIR), has_dayz_preflight_skill=_PREFLIGHT_IMPORTED,
    )
...
def preflight() -> PreflightResponse:
    ...
    if not p_drive: errors.append("P:\\ is not mounted. Run /dayz-mount-p or open DayZ Tools.")
    tools_path: Optional[str] = None
    if find_dayz_tools is not None:
        try:
            found = find_dayz_tools()
            if found: tools_path = str(found)
            else: warnings.append("DayZ Tools not detected.")
        except Exception as e: warnings.append(f"DayZ Tools resolver: {e}")
    ...
    if find_vanilla_data is not None:
        try:
            ...
            else: warnings.append("Vanilla data not detected.")
        except Exception as e: warnings.append(f"Vanilla data resolver: {e}")
    workshop_ok = _check_workshop_junction()
    if p_drive and not workshop_ok: warnings.append("P:\\Mods\\ junction missing.")
...
def _summarize_mod(path: Path) -> ModSummary:
    ...
    return ModSummary(name=path.name, path=str(path), has_config_cpp=config_cpp,
                      has_pboprefix=pboprefix, has_p_junction=p_junction, last_modified=mtime)


@app.get("/api/mods", response_model=ModListResponse)
def list_mods() -> ModListResponse:
    if not WORKSPACE_DIR.exists(): return ModListResponse(mods=[])
    mods: list[ModSummary] = []
...
async def _tail_watch_log_async(request: Request, since_pos: int = 0):
    if WATCH_LOG.exists():
        try:
            with WATCH_LOG.open("r", encoding="utf-8", errors="replace") as f: lines = f.readlines()
        ...
        try:
            if not WATCH_LOG.exists():
                if time.time() - last_heartbeat > 15:
                    yield _format_sse({"ts": time.time()}, event="heartbeat"); last_heartbeat = time.time()
                await asyncio.sleep(0.5); continue
            size = WATCH_LOG.stat().st_size
            if size < since_pos: since_pos = 0
            if size > since_pos:
                with WATCH_LOG.open("rb") as f:
                    f.seek(since_pos); chunk = f.read(size - since_pos)
                since_pos = size
...
async def stream_watch_log(request: Request) -> StreamingResponse:
    return StreamingResponse(_tail_watch_log_async(request), media_type="text/event-stream",
...
class RunRecord:
    def __init__(self, mod_name: Optional[str], skill: str, cmd: list[str]):
        ...
        self.mod_name = mod_name; self.skill = skill; self.cmd = cmd
        self.started_at = time.time()
        self.proc: Optional[asyncio.subprocess.Process] = None
        self.lines: list[dict] = []; self.exit_code: Optional[int] = None
...
def _skill_python_args(skill_name: str, *args: str) -> list[str]:
    ...
    for path in candidates:
        if path.exists(): return [sys.executable, str(path), *args]
...
async def _start_run(mod_name: Optional[str], skill: str, cmd: list[str]) -> RunRecord:
    ...
    rec = RunRecord(mod_name, skill, cmd); _RUNS[rec.run_id] = rec; _ACTIVE_BY_KEY[key] = rec.run_id
    ...
    async def reader():
        ...
        if _ACTIVE_BY_KEY.get(key) == rec.run_id: _ACTIVE_BY_KEY.pop(key, None)
...
async def build_mod(mod_name: str, clean: bool = False) -> StartRunResponse:
    ...
    return StartRunResponse(run_id=rec.run_id, skill=rec.skill, args=[mod_name, *extra], started_at=rec.started_at)
...
async def _run_stream(request: Request, run_id: str):
    ...
    if rec is None: yield _format_sse({"error": "unknown run"}, event="error"); return
        ...
        while True:
            ...
            if line is None:
                if time.time() - last_hb > 15:
                    yield _format_sse({"ts": time.time()}, event="heartbeat"); last_hb = time.time()
                continue
...
def kill_run(run_id: str) -> dict:
    ...
    try: rec.proc.kill(); return {"ok": True, "run_id": run_id}
...
def _write_port_file(port: int) -> None:
    ...
    PORT_FILE.write_text(json.dumps({"port": port, "pid_started_at": _started_at}), encoding="utf-8")
