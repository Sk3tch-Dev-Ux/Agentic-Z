...
export function Dashboard() {
  ...
  const activeRuns = useQuery({ queryKey: ["activeRuns"], queryFn: Api.activeRuns,
    refetchInterval: 2000 });
  const notify = useNotifyOnError();

  function onError(ev: WatchLogEvent) {
    notify(ev);
  }
  ...
  return (
    <div className="flex-1 overflow-auto p-6 space-y-6 min-h-0">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <StatCard label="Mods" value={mods.data?.mods.length ?? "—"} icon={<Zap className="w-4 h-4" />} sub="in workspace/" />
        <StatCard
          label="Active runs"
          value={activeRuns.data?.runs.length ?? 0}
          icon={<Activity className="w-4 h-4" />}
          sub={(activeRuns.data?.runs ?? []).map((r) => r.skill).slice(0, 2).join(", ") || "idle"}
        />
        <StatCard label="RAG (D4)" value="—" icon={<Search className="w-4 h-4" />} sub="indexed chunks" muted />
        <StatCard label="Proposals (D5)" value="—" icon={<Lightbulb className="w-4 h-4" />} sub="pending review" muted />
      </div>
...
function StatCard({
  ...
  return (
    <div className={"panel p-4 " + (muted ? "opacity-60" : "")}>
      <div className={"flex items-center gap-2 text-sm font-medium " + (muted ? "text-muted" : "text-accent-bright")}>
        {icon} {label}
      </div>
